...
//            ->toArray();
    ...
    public function getUserTransactions($user, array $filters, int $perPage = 15)
    {
        ...
        // If you prefer to show newest first in API/UI, reverse the collection AFTER calculation
    ...
    {
        $transactions = $this->getUserTransactions($user, $filters, 100); //Large Limit for export
        ...
        $callback = function () use ($transactions) {
            ...
            foreach ($transactions as $transaction) {
                fputcsv($file, [
                    // Convert string timestamp to Carbon for formatting
                    \Carbon\Carbon::parse($transaction['created_at'])->format('Y-m-d H:i:s'),
                    strtoupper($transaction['direction']),
                    $this->getTransactionDescription($transaction), // or your getTransactionDescription($transaction) adapted for arrays
                    $transaction['amount'],
                    $transaction['running_balance'] ?? 0, // or adapted helper function
                    $transaction['ref_id'] ?? 'N/A',
                ]);
            }
