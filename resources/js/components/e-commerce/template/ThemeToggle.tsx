...
export default function ThemeToggle() {
  ...
  if (!mounted) {
    return (
      <div className="relative w-14 h-6 bg-gradient-to-r from-emerald-200/80 to-green-200/80 dark:from-emerald-900/80 backdrop-blur-sm rounded-full shadow-lg border border-emerald-200/50 dark:border-emerald-700/50">
  ...
  return (
    ...
    >
      ...
      {/* Toggle Button */}
      <div
        className={`absolute top-0 w-6 h-6 bg-white dark:bg-gray-800 rounded-full shadow-md flex items-center justify-center transition-all duration-300 transform group-hover:scale-110 ${
          appearance === "light"
      ...
      <div className="absolute inset-0 flex items-center justify-between px-1.5 pointer-events-none">
        <span
          className={`text-xs transition-opacity duration-300 ${
            appearance === "light" ? "opacity-0" : "opacity-40"
          }`}
        >
          ☀️
        </span>
        <span
          className={`text-xs transition-opacity duration-300 ${
            appearance === "light" ? "opacity-40" : "opacity-0"
          }`}
        >
          🌙
        </span>
